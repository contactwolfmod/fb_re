...
/**
 * One Playwright-controlled Facebook Messenger session: login, poll for new
 * messages, reply via AI. Each instance is fully independent (its own
 * browser, session, dedupe state, persisted cookies file) so the same class
 * can back both the single shared admin bot and per-customer tenant bots.
 */
export class FacebookBotEngine {
  private opts: EngineOptions;
  private browserStatePath: string;
  private autostartFlagPath: string;

  private browser: Browser | null = null;
  private bContext: BrowserContext | null = null;
  private bPage: Page | null = null;
  ...
  constructor(opts: EngineOptions) {
    ...
    this.browserStatePath = path.join(STATE_BASE, `browser-state${suffix}.json`);
  ...
  private loadBrowserState(): object | null {
    try {
      if (fs.existsSync(this.browserStatePath)) {
        const raw = fs.readFileSync(this.browserStatePath, "utf8");
        const state = JSON.parse(raw);
        this.blog("info", { path: this.browserStatePath }, "Loaded saved browser state");
        return state;
      }
    } catch (e) {
      this.blog("warn", { err: String(e) }, "Could not load browser state — will use raw cookies");
    }
    return null;
  }

  private async saveBrowserState(ctx: BrowserContext): Promise<void> {
    try {
      const state = await ctx.storageState();
      fs.mkdirSync(path.dirname(this.browserStatePath), { recursive: true });
      fs.writeFileSync(this.browserStatePath, JSON.stringify(state));
      this.blog("info", {}, "Browser state saved (cookies + E2EE keys)");
    } catch (e) {
      this.blog("warn", { err: String(e) }, "Could not save browser state");
    }
  }
  ...
  private async extractDtsg(page: Page): Promise<string> {
  ...
  private async extractUID(page: Page): Promise<string> {
    return page.evaluate(() => {
      const scripts = Array.from(document.querySelectorAll("script"));
      for (const s of scripts) {
        const t = (s as HTMLScriptElement).textContent ?? "";
  ...
  private async processGraphQLText(text: string, source: string) {
    ...
    for (const item of candidates) {
      ...
      if (!Array.isArray(item) && item.error !== undefined) {
        ...
        this.blog(isCritical ? "warn" : "info", {
          source,
        ...
        if (item.payload) await this.processDataNode(item.payload, source);
        continue;
      }

      // Format 1: messenger.com — {"data": {"viewer": ...}} or {"data": {"message_thread": ...}}
      await this.processDataNode(item?.data, source);

      // Format 2: graphqlbatch — {"o0": {"data": {...}}}
      if (item?.o0) await this.processDataNode(item.o0?.data, source);

      // Format 3: array wrapper — [{"data":{...}}, ...]
      if (Array.isArray(item)) {
        for (const el of item) {
          await this.processDataNode(el?.data, source);
          if (el?.o0) await this.processDataNode(el.o0?.data, source);
        }
      }
    }
  }

  private async processDataNode(data: any, source: string) {
    if (!data || this.stopSignal) return;

    const keys = Object.keys(data);
    if (keys.length > 0) {
      this.blog("info", { keys, source }, "GraphQL data keys found");
    }

    // Thread list: viewer.message_threads.nodes
    const threadNodes: any[] = data?.viewer?.message_threads?.nodes ?? [];
    if (threadNodes.length > 0) {
      this.blog("info", { count: threadNodes.length, source }, "Thread list found");
    }
    ...
    for (const thread of threadNodes) {
      ...
      for (const msg of embeddedMsgs) {
        await this.checkAndHandleMsg(msg, threadID, threadType);
      }
    }
    ...
    if (threadData) {
      ...
      if (msgs.length > 0) {
        this.blog("info", { threadID, msgCount: msgs.length, source }, "Thread messages found");
      }
  ...
  private hasImageAttachment(msg: any): boolean {
    const attachments = [
      ...(msg.attachments ?? []),
  ...
  private async checkAndHandleMsg(msg: any, threadID: string, threadType: string) {
    if (!msg || this.stopSignal) return;
    ...
    if (senderId === this.sessionUID) return;

    const body: string = msg.message?.text ?? msg.body ?? "";
    const isImageOnly = !body.trim() && this.hasImageAttachment(msg);
    if (!body.trim() && !isImageOnly) return;

    this.lastSeenTimestamp.set(threadID, Math.max(lastSeen, ts));
  ...
  private async handleMessage(
    ...
    if (threadType !== "ONE_TO_ONE") {
      this.blog("info", { threadId, threadType }, "Skipping group/community thread (DM-only mode)");
      return;
    }
    ...
    if (!resolved) {
      if (this.bPage && this.opts.onUnresolvedThread) {
        ...
        if (fallbackMsg) {
          ...
          await this.sendFbMessageUI(this.bPage, threadId, fallbackMsg);
        } else {
    ...
    if (!this.bPage) {
      this.blog("warn", { threadId }, "bPage is null — cannot reply");
      return;
    }
    ...
    try {
      ...
      await this.sendFbMessageUI(this.bPage, threadId, reply);
      this.opts.state.messagesHandled++;
      this.blog("info", { threadId, senderName }, "Reply sent ✓");
    } catch (err) {
      this.blog("error", { err: String(err), threadId }, "Reply failed — sending fallback");
      try {
        await this.sendFbMessageUI(this.bPage, threadId, "Xin lỗi, tôi đang gặp sự cố kỹ thuật. Vui lòng thử lại sau.");
  ...
  private async sendFbMessageUI(page: Page, threadID: string, text: string): Promise<void> {
    ...
    if (!isCorrectThread) {
      this.blog("info", { threadID }, "Navigating to thread for send");
    ...
    this.blog("info", { threadID, sel: result.sel, execOk: result.ok, editorLen: result.len }, "Text inserted into editor");
    ...
    if (afterLen > 0) {
      ...
      this.blog("warn", { threadID, afterLen }, "Enter did not send — trying send button");
    ...
    this.blog("info", { threadID, len: text.length }, "Message sent via UI ✓");
  }
  ...
  private async scrapeConversationDOM(page: Page, initOnly = false): Promise<void> {
    if (this.stopSignal) return;
    ...
    if (isLoginPage) {
      this.blog("error", { uniqueLabels: result.uniqueLabels }, "Login overlay detected — cookies expired or invalid!");
      this.opts.state.status = "error";
      this.opts.state.error = "Phiên đăng nhập hết hạn. Vui lòng vào Settings → dừng bot → cập nhật cookies mới → khởi động lại.";
    ...
    if (result.isGroupThread) {
      this.blog("info", { threadID: result.threadID, reason: result.groupDetectReason }, "Skipping group thread (DM-only mode)");
      return;
    }
    ...
    if (result.rowMsgs.length > 0) {
      this.blog("info", { sample: result.rowMsgs }, "DOM row messages sample");
    }
    if (result.sentByMsgs.length > 0) {
      this.blog("info", { sample: result.sentByMsgs }, "DOM sentBy messages sample");
    }
    ...
    if (initOnly) {
      ...
      for (const m of result.sentByMsgs as any[]) {
        ...
        this.blog("info", { ts, isToday, msgKey: msgKey.slice(0, 60) }, "initOnly: time check");
        if (isToday) {
          keptRecent++;
        } else {
          this.repliedMessageIds.add(msgKey);
          markedOld++;
        }
      }
      this.blog("info", { threadID, markedOld, keptRecent }, "First poll: old msgs marked seen, today's msgs queued for reply");
      return;
    }
    ...
    for (const m of result.sentByMsgs as any[]) {
      ...
      if (this.repliedMessageIds.has(msgKey)) continue;

      this.blog("info", { threadID, senderName, text: text.slice(0, 80) }, "DOM: new message detected");
      const ts = Date.now();
      await this.checkAndHandleMsg(
        {
          message: { text },
  ...
  private async setupInterceptor(page: Page) {
    ...
    await page.route("**/api/graphql/**", async (route, request) => {
      try {
        ...
        this.processGraphQLText(body, request.url()).catch(() => {});
        await route.fulfill({ response, body });
      } catch (err: any) {
        this.blog("warn", { err: err?.message }, "graphql route error");
    ...
    page.on("response", async (resp) => {
      ...
      if (!p.includes("graphql") && !p.includes("messaging")) return;

      try {
        const text = await resp.text();
        this.processGraphQLText(text, p).catch(() => {});
      } catch (_) {}
    });

    this.blog("info", {}, "messenger.com interceptors registered");
  }

  // ---------------------------------------------------------------------------
  // Extract thread IDs visible in the sidebar/inbox
  // ---------------------------------------------------------------------------
  private async getSidebarThreadIDs(page: Page): Promise<string[]> {
    return page.evaluate(() => {
  ...
  private startPollLoop() {
    ...
    const doPoll = async () => {
      if (this.stopSignal || !this.bPage) return;

      try {
        const currentUrl = this.bPage.url();
        ...
        if (firstPoll || !alreadyOnMessages || fastPollCount % FULL_RELOAD_EVERY === 0) {
          ...
          await this.bPage.goto("https://www.facebook.com/messages/", {
            waitUntil: "domcontentloaded",
            timeout: 25000,
          });
          await this.bPage.waitForTimeout(1200);
        } else {
          // Fast reload — stays on same URL, triggers GraphQL interceptors
          await this.bPage.reload({ waitUntil: "domcontentloaded", timeout: 15000 });
          await this.bPage.waitForTimeout(800);
        }

        fastPollCount++;

        const dtsg = await this.extractDtsg(this.bPage);
        if (dtsg) this.sessionDtsg = dtsg;

        // Discover conversations from the sidebar
        const threadIDs = await this.getSidebarThreadIDs(this.bPage);
        this.blog("info", { threadIDs, initOnly: firstPoll, fastPollCount }, "Poll: discovered conversations");

        if (firstPoll) {
          // On first poll: visit every thread to seed lastSeenTimestamp (no reply)
          for (const tid of threadIDs) {
            if (this.stopSignal) break;
            await this.bPage.goto(`https://www.facebook.com/messages/t/${tid}/`, {
              waitUntil: "domcontentloaded",
              timeout: 20000,
            });
            await this.bPage.waitForTimeout(900);
            await this.scrapeConversationDOM(this.bPage, true);
          }
        } else {
          ...
          const threadIDsWithNew = await this.bPage.evaluate(() => {
          ...
          this.blog("info", { badgeThreads: threadIDsWithNew, topThreads, toVisit }, "Poll: threads to visit");
          ...
          for (const tid of toVisit) {
            ...
            await this.bPage.waitForTimeout(700);
            await this.scrapeConversationDOM(this.bPage, false);
            // Return to inbox after each thread
            await this.bPage.goto("https://www.facebook.com/messages/", {
              waitUntil: "domcontentloaded",
              timeout: 15000,
            });
            await this.bPage.waitForTimeout(600);
          }
        }
        ...
        this.blog("error", { err: msg }, "Poll error");
        ...
        if (isFatal || isCrash) {
          this.blog("warn", { isFatal, isCrash }, "Browser closed/crashed — attempting full browser restart");
          try {
            // Tear down old browser completely
            try { this.browser?.close(); } catch {}
            this.browser = null; this.bContext = null; this.bPage = null;
            ...
            const savedState = this.loadBrowserState();
            this.bContext = await this.browser.newContext({
            ...
            await this.bContext.addInitScript(() => {
              Object.defineProperty(navigator, "webdriver", { get: () => false });
              // @ts-ignore
              if (!window.chrome) window.chrome = { runtime: {} };
            });

            this.bPage = await this.bContext.newPage();
            await this.setupInterceptor(this.bPage);
            this.blog("info", {}, "Browser restarted successfully — resuming poll");
            if (!this.stopSignal) this.pollTimer = setTimeout(doPoll, 5000);
          } catch (restartErr: any) {
            this.blog("error", { err: restartErr?.message }, "Browser restart failed — stopping bot");
            this.opts.state.status = "error";
            this.opts.state.error = "Không thể khởi động lại browser. Vui lòng dừng và khởi động lại bot.";
          }
          return;
        }
      }
    ...
    this.pollTimer = setTimeout(doPoll, 4000);
  }
  ...
  private async finishBotSetup(fallbackUID?: string): Promise<void> {
    if (!this.bPage || !this.bContext) throw new Error("Browser not initialized");

    this.blog("info", {}, "Navigating to facebook.com/messages/...");
    await this.bPage.goto("https://www.facebook.com/messages/", {
      waitUntil: "domcontentloaded",
      timeout: 30000,
    });

    const fbUrl = this.bPage.url();
    this.blog("info", { url: fbUrl }, "Navigation result");
    ...
    if (fbUrl.includes("/login")) {
      try { fs.unlinkSync(this.browserStatePath); } catch (_) {}
      throw new Error("Đăng nhập thất bại hoặc cookie đã hết hạn. Vui lòng thử lại.");
    }

    await this.bPage.waitForTimeout(3000);

    let uid = await this.extractUID(this.bPage);
    if (!uid && fallbackUID) uid = fallbackUID;
    this.sessionUID = uid;

    const dtsg = await this.extractDtsg(this.bPage);
    if (!dtsg) throw new Error("Không thể lấy token bảo mật (fb_dtsg). Vui lòng thử lại.");
    this.sessionDtsg = dtsg;

    await this.saveBrowserState(this.bContext);
    ...
    this.blog("info", { uid, dtsgPrefix: this.sessionDtsg.substring(0, 10) + "..." }, "Session ready");
  ...
  async submit2FACode(code: string): Promise<void> {
    ...
    const submitDone = await this.bPage.evaluate(() => {
      const selectors = [
    ...
    await this.bPage.waitForURL(
      (url) => !url.toString().includes("checkpoint") && !url.toString().includes("two_step"),
      { timeout: 20000 }
    ).catch(() => {});

    await this.bPage.waitForTimeout(2000);

    const postUrl = this.bPage.url();
    if (
    ...
    this.blog("info", { url: postUrl }, "2FA verified — continuing bot setup");
    this.opts.state.error = null;

    await this.finishBotSetup();
  }
  ...
  async start(credentials: LoginCredentials): Promise<void> {
    ...
    this.blog("info", {}, "Launching Chromium");
    ...
    if (credentials.type === "appstate") {
      ...
      this.blog("info", { count: fbCookies.length, hasSavedState: !!savedState }, "Cookies injected");
    }
    ...
    if (credentials.type === "credentials") {
      ...
      this.blog("info", { identifier: credentials.email }, "Performing credentials login via Playwright");

      await this.bPage.goto("https://m.facebook.com/login", {
        waitUntil: "domcontentloaded",
        timeout: 30000,
      });

      const emailInput = this.bPage.locator('input[name="email"], input[type="email"], #m_login_email');
      await emailInput.waitFor({ timeout: 15000 });
      await emailInput.fill(credentials.email);

      const passInput = this.bPage.locator('input[name="pass"], input[type="password"]');
      ...
      const submitted = await this.bPage.evaluate(() => {
        const selectors = [
          'button[name="login"]',
          'input[name="login"]',
      ...
      if (!submitted) {
        this.blog("info", {}, "No submit button found — pressing Enter on password field");
        await passInput.press("Enter");
      } else {
        this.blog("info", { submitted }, "Login button clicked");
      }

      await this.bPage.waitForURL((url) => !url.toString().includes("/login"), { timeout: 25000 }).catch(() => {});
      await this.bPage.waitForTimeout(2500);

      const loginUrl = this.bPage.url();
      const loginContent = await this.bPage.content();
      ...
      if (
        ...
        this.blog("info", { url: loginUrl }, "2FA detected — waiting for user OTP");
      ...
      this.blog("info", { url: loginUrl }, "Credentials login succeeded");
    }
    ...
    await this.finishBotSetup(fallbackUID);
  }
