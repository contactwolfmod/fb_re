// Resolves a Facebook profile link to its numeric user ID via the free
// id.traodoisub.com lookup service, with a safe manual fallback: the service
// sits behind Cloudflare's bot-challenge, so automated requests frequently get
// blocked outright. We treat that as a normal, expected failure mode (not a
// crash) and let the caller fall back to a directly-pasted numeric ID.
...
export async function resolveFacebookId(input: string): Promise<ResolveFbIdResult> {
  ...
  // Admin already has the numeric ID (e.g. lookup service was down last time) — skip the API call entirely.
  if (/^\d{5,20}$/.test(raw)) return { ok: true, id: raw };

  // "profile.php?id=NNNN" links already carry the numeric ID in the URL —
  // no need to round-trip through the (Cloudflare-flaky) lookup API for these.
  const profileIdParam = raw.match(/[?&]id=(\d{5,20})\b/);
  if (profileIdParam) return { ok: true, id: profileIdParam[1] };
  ...
  try {
    ...
    const candidate = data?.id ?? data?.uid ?? data?.data ?? data?.result ?? data?.fb_id;
    if (candidate) {
      const m = String(candidate).match(NUMERIC_ID);
      if (m) return { ok: true, id: m[1] };
    }
  } catch {
    // Not JSON — fall through to plain-text extraction below.
  }
